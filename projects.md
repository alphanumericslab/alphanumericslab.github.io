...
Ventricular repolarization does not follow heart rate instantaneously. When heart rate rises during exercise or stress and falls again during recovery, the QT interval and the T-wave amplitude lag behind the RR interval, so the same heart rate produces a different T wave depending on whether the heart is speeding up or slowing down. Conventional QT correction formulas (Bazett, Fridericia and others) ignore this memory, and T-wave amplitude is usually reduced to a single average value with no heart-rate adjustment at all.
